...
class SSSP:
    ...
    def search(self):
        ...
        while True:
            ...
            while frontier:
                ...
                for weight, q_state_to in self.roadmaps[i][q_state_from]: # 자기 자신을 포함해야된다.
                    q_prime = list()
                    for q_state in s_q:
                        q_prime.append(q_state)

                    q_prime[i] = q_state_to  # 로봇을 이동

                    if (q_prime, i_prime) not in explored and not self.collide(s_q, q_prime, i):
                        score = self.calculate_score(q_prime)  # a*로 최소 거리 계산
    ...
    def draw_result(self, s_):
        ...
        while next_s:
            ...
            step = 5
            for j, q_state in enumerate(q):
                d, theta = self.rrt.calc_distance_and_angle(q_state, next_s[3][1][j])
    ...
    def reconstruct_paths(self, s_):
        ...
        while next_s[3]:
            for i, node in enumerate(next_s[1]):
                self.paths[i].append([[node.x, next_s[3][1][i].x], [node.y, next_s[3][1][i].y]])
            next_s = next_s[3]
